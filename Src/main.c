...
void initialize_eeprom_with_defaults() {
    ...
    // New parameters added in 2.19 (5-12)
    eepromBuffer.max_ramp = EEPROM_DEFAULT_MAX_RAMP;
    eepromBuffer.minimum_duty_cycle = EEPROM_DEFAULT_MINIMUM_DUTY_CYCLE;
    eepromBuffer.disable_stick_calibration = EEPROM_DEFAULT_DISABLE_STICK_CALIBRATION;
    eepromBuffer.absolute_voltage_cutoff = EEPROM_DEFAULT_ABSOLUTE_VOLTAGE_CUTOFF;
    eepromBuffer.current_P = EEPROM_DEFAULT_CURRENT_P;
    eepromBuffer.current_I = EEPROM_DEFAULT_CURRENT_I;
    eepromBuffer.current_D = EEPROM_DEFAULT_CURRENT_D;
    eepromBuffer.active_brake_power = EEPROM_DEFAULT_ACTIVE_BRAKE_POWER;

    // reserved (13-16)
    for (uint8_t i = 0; i < sizeof(eepromBuffer.reserved_eeprom_3); i++) {
